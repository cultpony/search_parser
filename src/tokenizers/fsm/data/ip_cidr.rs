...
use super::ipv6_cidr::{IPv6Address, IPv6Network};
...
impl FSMStateMatcher for IPAddress {
    fn matches(self, inp: &str) -> Option<u8> {
        IPv4Network
            .matches(inp)
            .or_else(|| IPv4Address.matches(inp))
            .or_else(|| IPv6Network.matches(inp))
            .or_else(|| IPv6Address.matches(inp))
    }

    fn maximum_bound(self) -> Option<u8> {
        IPv4Address::maximum_bound(IPv4Address).max(IPv4Network::maximum_bound(IPv4Network))
    }
}
...
impl FSMStateMatcher for IPv4Network {
    fn matches(self, inp: &str) -> Option<u8> {
        ...
        let ip_prefix = IPv4Address.matches(inp)?;
...
mod test {
    ...
    pub fn test_ipv4_matcher() {
        ...
        test!(exact "127.0.0.1");
        test!(exact "127.0.0.127");
        test!(exact "255.255.255.255");
        test!(extra "127.0.0.127" "a");
        test!(extra "127.0.0.12" "a");
        test!(extra "127.0.0.1" "a");
        test!(bad "300.0.0.1");
        test!(bad "300.0.0.1");
        test!(bad "300.0.0.1");
        test!(bad "256.0.0.1");
        test!(bad "127.256.0.1");
        test!(bad "127.255.256.1");
        test!(bad "127.255.255.256");
        test!(bad "127.256.0.1000");
        test!(bad "127.256.1000.1");
        test!(bad "127.2560.100.1");
        test!(bad "a.2.3.4");
        test!(bad "127.255.a.1");
        test!(bad "test");
        test!(extra "0.0.0.0" "a0");
        test!(extra "0.100.0.0" "a0");
    }
    ...
    proptest::proptest! {
        #[test]
        fn proptest_ipv4_matcher(d1 in 0..255u8, d2 in 0..255u8, d3 in 0..255u8, d4 in 0..255u8, g in "[a-zA-Z,) ][0-5]") {
        ...
        #[test]
        fn proptest_ipv4_network_matcher(d1 in 0..255u8, d2 in 0..255u8, d3 in 0..255u8, d4 in 0..255u8, n in 0..32u8, g in "[a-zA-Z,) ][0-5]") {
